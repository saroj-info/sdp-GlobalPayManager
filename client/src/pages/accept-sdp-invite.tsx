...
export default function AcceptSdpInvite() {
  ...
  useEffect(() => {
    const fetchInvite = async () => {
      ...
      try {
        const response = await fetch(`/api/sdp-users/invite/${token}`);
        if (!response.ok) {
  ...
  const validateForm = (): boolean => {
    ...
    return !errors.password && !errors.confirmPassword && !errors.verificationCode;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiRequest("POST", "/api/sdp-users/accept", {
        token,
        password,
        totpSecret,
  ...
  const roleDisplayName = roleDisplayNames[inviteData?.sdpRole] || inviteData?.sdpRole;
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-orange-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-2xl">
          <CardHeader>
            <CardTitle>Join SDP Global Pay Team</CardTitle>
            <CardDescription>
